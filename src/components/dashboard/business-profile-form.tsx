...
import { businessSchema, type BusinessInput } from "@/lib/validation/business";
...
export function BusinessProfileForm({ business }: { business: Business }) {
  ...
  const onSubmit = (values: BusinessInput) => {
    startTransition(async () => {
      ...
      toast.success("Biznes ma'lumotlari saqlandi");
      setEditing(false);
    });
  };
      ...
      <div className="mb-6 rounded-card border border-border bg-surface p-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            {business.address ? <p className="mt-2 text-sm text-slate-500">{business.address}</p> : null}
            {business.phone ? <p className="text-sm text-slate-500">{business.phone}</p> : null}
          </div>
          <button
            onClick={() => setEditing(true)}
            className="flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-sm font-medium text-slate-600 hover:border-accent-500 hover:text-accent-700"
          >
    ...
    >
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <input {...register("slug")} className="w-full rounded-md border border-border px-3 py-2 text-sm" />
        ...
        <div>
          ...
          <input {...register("phone")} className="w-full rounded-md border border-border px-3 py-2 text-sm" placeholder="+998901234567" />
          {errors.phone ? <p className="mt-1 text-xs text-red-600">{errors.phone.message}</p> : null}
        </div>
        <div className="md:col-span-2">
          <label className="mb-1 block text-sm font-medium text-slate-700">Tavsif</label>
          <textarea {...register("description")} rows={2} className="w-full rounded-md border border-border px-3 py-2 text-sm" />
        </div>
      </div>
      <div className="flex gap-2">
        ...
        <button type="button" onClick={() => setEditing(false)} className="rounded-md border border-border px-4 py-2 text-sm font-medium text-slate-600">
          Bekor qilish
        </button>
      </div>
    </form>
